 ...
 * offer — the abandoned-cart recovery codes (BACK15-…) issued by
 * sendRecoveryEmails(), plus the owner-only checkout test codes (TEST99-…)
 * issued by issueOwnerTestCode() — both in apps-script.gs. Encoding the percent
 * in the code string keeps the trust boundary intact: Apps Script decides
 * *validity* (right email, unused, not expired) but this file still decides the
 * *percent*, so a compromised Apps Script endpoint can never invent a bigger
 * discount. Keep the prefixes IN SYNC with RECOVERY_CODE_PREFIX and
 * OWNER_TEST_CODE_PREFIX in apps-script.gs.
 */
...
const PREFIX_PERCENTS = [
  ...
  // Owner-only checkout test code: pays ~₪2 on a ₪198 package so the real SUMIT
  // flow (redirect → payment → callback → markUsed → confirmation email) can be
  // exercised end-to-end with a live card. NOT a customer offer. Safe to expose
  // the prefix: minting a working code still requires appending a row to the
  // "Discount Signups" sheet bound to an OWNER_EMAILS address, which only the
  // script owner can do (issueOwnerTestCode() in apps-script.gs — never reachable
  // over doPost). Single-use and short-lived, so a leaked code dies on first use.
  ['TEST99-', 99],
];
