...
async function sendPurchaseEvent(input) {
  ...
  try {
    ...
    if (!metaRes.ok) {
      // CAPI-REJECTED is a deliberate, greppable marker: this failure is
      // otherwise completely silent (the HTTP route still returns 200 and
      // payment-callback never blocks the redirect on it), so a token that lacks
      // permission on the configured pixel can burn ad spend for weeks while
      // every Purchase is quietly 400'd. Search Vercel logs for CAPI-REJECTED,
      // or call /api/tracking-health for a read-only check that sends no event.
      const err = data && data.error;
      console.error('[meta-capi] CAPI-REJECTED status=' + metaRes.status +
        ' code=' + ((err && err.code) || '?') +
        ' subcode=' + ((err && err.error_subcode) || '?') +
        ' pixel=' + pixelId +
        ' msg=' + ((err && err.message) || JSON.stringify(data)));
...
/**
 * READ-ONLY check that META_CAPI_TOKEN actually has permission on META_PIXEL_ID.
 *
 * This is a plain Graph API GET on the pixel — it sends NO event, so it can be
 * called as often as you like without putting a single test Purchase into your
 * conversion data (the reason not to "just fire a test event" to check).
 *
 * A token minted for a DIFFERENT pixel fails here exactly the way it fails on a
 * real Purchase: code 100 / error_subcode 33, "does not exist, cannot be loaded
 * due to missing permissions". That is the failure that silently stopped Meta
 * from ever learning who buys.
 *
 * Never throws. Returns { ok, configured, pixelId, pixelName?, ... }.
 */
async function checkPixelAccess() {
  const pixelId = process.env.META_PIXEL_ID;
  const token = process.env.META_CAPI_TOKEN;
  if (!pixelId || !token) {
    return {
      ...
      missing: [!pixelId && 'META_PIXEL_ID', !token && 'META_CAPI_TOKEN'].filter(Boolean),
    };
  }
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return {
        ...
        // Meta's message names the cause; safe to surface (it contains no token).
        error: err.message || 'Meta rejected the pixel lookup',
        hint: (err.code === 100 && err.error_subcode === 33)
          ? 'META_CAPI_TOKEN has no permission on this pixel — regenerate the token FOR this pixel.'
          : undefined,
      };
    }
    return { ok: true, configured: true, pixelId, pixelName: data && data.name };
  } catch (e) {
    const aborted = e && e.name === 'AbortError';
    return { ok: false, configured: true, pixelId, error: aborted ? 'Meta timeout' : 'Pixel lookup failed' };
  } finally {
    clearTimeout(timer);
  }
}

module.exports = { sendPurchaseEvent, checkPixelAccess };
